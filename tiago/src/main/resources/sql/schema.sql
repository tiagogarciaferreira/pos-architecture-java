...
CREATE TABLE tb_genres (
    ...
    name VARCHAR(50) NOT NULL CHECK (LENGTH(name) >= 4 AND LENGTH(name) <= 50),
...
CREATE TABLE tb_actor_awards (
    ...
    award_name VARCHAR(100) NOT NULL CHECK (LENGTH(award_name) >= 3 AND LENGTH(award_name) <= 100),
    created TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
    modified TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
CREATE TABLE tb_movies (
    ...
    imdb_id VARCHAR(12) NOT NULL CHECK (LENGTH(imdb_id) >= 9 AND LENGTH(imdb_id) <= 11), --AND (imdbId ~ '^tt\\d{7,9}$')
